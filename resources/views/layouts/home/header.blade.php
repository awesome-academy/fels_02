    ...
    <div class="container">
        ...
        <div class="navbar-collapse collapse" id="bs-example-navbar-collapse-1">
            <ul class="nav navbar-nav">
                ...
                @else
                    <li class="dropdown">
                        <a href="javascript:void(0)" class="dropdown-toggle" data-toggle="dropdown"><i class="fa fa-user"></i><span> {{ Auth::user()->username }}</span></a>
                        <ul class="dropdown-menu">
                            <li>
                                <a class="dropdown-item" href="javascript:void(0)" id="logout" onclick="return false">@lang('messages.btn_logout')</a>
                            </li>
                            <li>
                                <a class="dropdown-item" href="{{Route('profile.index')}}" id="edit" >@lang('messages.profile')</a>
                            </li>
                        </ul>
                    </li>
                @endguest
                ...
                <li class="dropdown">
                    {!! Form::open(['method' => 'POST', 'route'=>['switch.lang'], 'class' => 'form-lang']) !!}
                    <span class="icon-lang"><i class="fa fa-globe"></i></span>
                    {!! Form::select
                    (
                        'locale',
                                        ...
                                        <ul class="nav-list list-inline">
                                            @foreach($displayLessons as $key => $lesson)
                                                @if($lesson->topic_id == $topic->topic_id)
                                                    <li><a href="#"><img src="/images/lessons/{{ $lesson->picture }}" class="img-responsive in-header"/></a></li>
                                                @endif
                                            @endforeach
                                        </ul>
                                    </div>
